...
Swagmode_2.Text = "discord.gg/nbdleakss"
...
UIS.InputBegan:Connect(function(key,b) 
    ...
    if key.KeyCode == Enum.KeyCode.F and not b then
        if HideBlockColor.BackgroundColor3 == green or AutoBlockColor.BackgroundColor3 == green then
            HoldingF = true
            player.Character:FindFirstChildWhichIsA('Humanoid').Name = 'NBDLEAKS ON TOP'
...
SwagUsers.MouseButton1Click:Connect(function()
    ...
    if TOTAL > 0 then
        announce('Found','A total of ' .. tostring(TOTAL) .. ' using this script in this server', 3)
    else
        announce('D:', 'no script vusers found', 3)
    end
end)
...
elseif table.find(swagmins, player.UserId) then
    ...
    IsMod = false
    DisableMod = false
    announce('Commands Loaded', 'join discord.gg/nbdleakss for a list of commands', 10)
end
end)
